      ...
      <div className="max-w-7xl mx-auto px-6 md:px-10">
        <div className="text-center max-w-4xl mx-auto mb-24">
            <div className="inline-flex items-center gap-3 mb-8 bg-brand-primary/10 px-4 py-2 rounded-full border border-brand-primary/20">
                ...
                <div className="text-[9px] font-black uppercase tracking-[0.4em] text-brand-primary">Wellness_Care_Node_v1</div>
            </div>
            <h1 className="text-6xl md:text-8xl font-black text-white uppercase tracking-tighter mb-10 leading-[0.85]">
                Human Care. <br /> <span className="text-gradient">Neural</span> <br /> Scale.
            </h1>
            <p className="text-xl md:text-2xl text-white/40 font-medium max-w-2xl mx-auto leading-relaxed">
                Automating the patient journey for high-growth wellness clinics. Deploying Care-Agents that resolve booking churn and triage inquiries autonomously.
            </p>
        </div>
        ...
        <div className="grid lg:grid-cols-2 gap-8 mb-20">
            <div className="p-12 bg-white/5 border border-white/10 rounded-2xl relative overflow-hidden group">
                ...
                <div className="space-y-8">
                    <CarePulse label="Automated_Bookings" percentage={84} />
                    <CarePulse label="Churn_Mitigation" percentage={92} />
                    <CarePulse label="Triage_Accuracy" percentage={83} />
                </div>
            </div>

            <div className="grid grid-cols-2 gap-8">
                {[
                    { title: "Voice Triage", icon: Volume2, desc: "Process patient inquiries via autonomous voice nodes." },
                    { title: "Smart Scheduling", icon: Calendar, desc: "Automate appointment loops and churn follow-ups." },
                    { title: "Patient Registry", icon: Users, desc: "Synchronize patient data across clinical silos." },
                    { title: "Secure Health", icon: ShieldCheck, desc: "POPIA compliant data sovereignty for sensitive health info." }
                ].map((item, i) => (
        ...
        <div className="border-t border-white/5 pt-20 flex flex-col items-center">
            <div className="text-[10px] font-black uppercase tracking-[0.5em] text-white/20 mb-10">Antigravity Accelerated Deployment</div>
            <div className="flex flex-wrap justify-center gap-12">
                <button className="btn-primary group">
                    <span className="flex items-center gap-3">Deploy Care-Node <Calendar className="w-4 h-4" /></span>
                </button>
